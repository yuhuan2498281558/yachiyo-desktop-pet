...
当前测试基线为 27 项通过，覆盖 Codex 状态分类、台词去重、视线方向、偏好持久化、诊断日志和 Sprite 动作队列。`npm run check` 检查全部 JavaScript/CJS 源文件，含 `src/preferences.cjs`、`src/diagnostics.cjs` 与 `src/renderer/sprite-actions.js`。涉及窗口、托盘、拖动、动画或场景时仍需在 Windows 上手工验证。
