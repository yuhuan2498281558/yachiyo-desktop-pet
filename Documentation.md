...
当前测试覆盖 Codex 状态分类、台词去重、视线方向、偏好持久化和诊断日志。`npm run check` 检查全部 JavaScript/CJS 源文件，含 `src/preferences.cjs` 与 `src/diagnostics.cjs`。涉及窗口、托盘、拖动、动画或场景时仍需在 Windows 上手工验证。
